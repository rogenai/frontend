...
export class Player extends Entity {
        ...
        this.scene.input.on('pointerdown', (pointer: any) => {
            ...
            this.scene.time.delayedCall(400, () => {
                this.scene.physics.collide(this.sword, (this.scene as Game).objects, (a, b) => {
                    ...
                    if ((this.scene as any).id === "tutorial") {
                        (b as Entity).health -= 50;
                        return;
                    }
    ...
    protected preUpdate(time: number, delta: number): void {
        ...
        if (this.state !== "idle" && (this.scene as any).id !== "tutorial") {
    ...
    collide(other: SpriteObject): void {
        if (other.type === "bullet") {
            if (this.scene) {
                ...
                else (this.scene as any).socket.emit("damage-got", 10);
            }
            other.destroy();
        }
    }
...
export class TutorialPlayer extends Entity {
    isJerk: boolean;
    isCooldownJerk: boolean;
    isDamage: boolean;
    isAttack: boolean;
